Shader "LiteNinja/MeshCraft/FilledTriangleWireframe"
{
    Properties
    {
        // Wire color with HDR support
        [HDR] _WireColor ("Wire color", Color) = (0, 0, 0, 1)
        // Fill color with HDR support
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f input_data) : SV_Target
            {
                ...
                fixed4 wire_color = fixed4(_WireColor.rgb, line_alpha);
                // Return the fill color for the internal part of the triangle
                fixed4 fill_color = fixed4(_FillColor.rgb, _FillColor.a);

                // Blend the wire color and the fill color
                return lerp(fill_color, wire_color, line_alpha);
            }
